...
ALTER TABLE public.spark_transactions ADD CONSTRAINT spark_transactions_tx_type_check CHECK (tx_type = ANY (ARRAY['referral_bonus','signup_bonus','circle_contribution','circle_payout','spark_trade_join','spark_exchange_buy','spark_exchange_sell','game_entry','game_win','admin_adjustment','manual_credit','referral_signup','referral_kyc_bonus','referral_admin_bonus','tender_unlock']));

DROP FUNCTION IF EXISTS public.unlock_tender(uuid, text);
...
BEGIN
  ...
  -- House convention: positive amount, direction implied by from_member (debit)
...
REVOKE ALL ON FUNCTION public.unlock_tender(uuid, text) FROM public, anon;
