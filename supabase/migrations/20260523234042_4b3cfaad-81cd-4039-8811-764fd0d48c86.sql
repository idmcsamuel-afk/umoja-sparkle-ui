...
AS $$
  WITH latest_per_tier AS (
    ...
    WHERE cb.member_id = auth.uid()
      AND cb.status IN ('active', 'vault', 'payment_pending', 'pending', 'paid')
    ORDER BY cb.tier,
      CASE cb.status
        WHEN 'active' THEN 1
        WHEN 'vault' THEN 2
  ...
  enriched AS (
    SELECT
      ...
      COALESCE(
        l.vault_end,
        CASE
          WHEN l.status IN ('active', 'vault') THEN
  ...
  SELECT
    ...
    (
      ...
      WHERE cb3.tier = e.tier
        AND cb3.status IN ('active', 'vault')
        AND (
          cb3.vault_end > now()
          OR cb3.vault_end IS NULL
        )
    ) AS total_active,
    CASE
...
AS $$
  SELECT cb.tier,
    ...
    FROM public.circle_bids cb
   WHERE cb.status IN ('active', 'vault')
     AND (cb.vault_end > now() OR cb.vault_end IS NULL)
   GROUP BY cb.tier;
$$;

GRANT EXECUTE ON FUNCTION public.circle_tier_stats() TO authenticated;